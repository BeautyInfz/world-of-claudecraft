...
import { Document, getBounds, NodeIO, type Primitive } from '@gltf-transform/core';
    ...
    keys.push(
      primitive
        ...
        .map((semantic) => {
          ...
          return `${semantic}:${accessor.getComponentType()}:${bytes.toString('hex')}`;
        })
        .join('|'),
    );
  }
...
describe('foliage field bark decimation', () => {
  ...
  for (const asset of EXPECTED) {
    it(`ships ${asset.model}_field with decimated bark and untouched leaves and textures`, async () => {
      ...
      expect(asset.bark.triangles).toBeLessThanOrEqual(asset.budget);
      ...
      expect(outputParts.others.map((primitive) => primitive.getMaterial()?.getName())).toEqual(
        sourceParts.others.map((primitive) => primitive.getMaterial()?.getName()),
      );
  ...
  it('simplifies only the bark primitive and drops the vertices it stops using', async () => {
    ...
    for (let row = 0; row < side; row++) {
      for (let col = 0; col < side; col++) positions.push(col, Math.sin(col * row) * 0.2, row);
    }
    const gridIndices: number[] = [];
    for (let row = 0; row < side - 1; row++) {
      for (let col = 0; col < side - 1; col++) {
        const a = row * side + col;
        gridIndices.push(a, a + side, a + 1, a + 1, a + side, a + side + 1);
      }
    }
    const accessor = (
    ...
    const mesh = document.createMesh().addPrimitive(bark).addPrimitive(leaf);
    ...
    const report = await decimateFoliageBarkDocument(document, {
      barkTriangleBudget: 60,
      simplifier: MeshoptSimplifier,
      encoder: MeshoptEncoder,
    });

    const { bark: finishedBark, others } = splitPrimitives(document);
    const barkTriangles = (finishedBark[0].getIndices()?.getCount() ?? 0) / 3;
    const barkVertices = finishedBark[0].getAttribute('POSITION')?.getCount() ?? 0;
    ...
    expect(barkVertices).toBeLessThan(side * side);
    expect(new Set(finishedBark[0].getIndices()?.getArray()).size).toBe(barkVertices);
    expect(primitiveTriangleAttributeFingerprint(others[0])).toBe(leafBefore);

    await expect(
      decimateFoliageBarkDocument(document, {
        barkTriangleBudget: barkTriangles,
        simplifier: MeshoptSimplifier,
        encoder: MeshoptEncoder,
      }),
    ).rejects.toThrow(/is not below/);
    leaf.setMaterial(document.createMaterial('Bark_Second'));
    await expect(
      decimateFoliageBarkDocument(document, {
        barkTriangleBudget: 1,
        simplifier: MeshoptSimplifier,
        encoder: MeshoptEncoder,
      }),
    ).rejects.toThrow(/Expected one bark primitive, found 2/);
  });
});
...
describe('foliage field bark consumers', () => {
  ...
  it('keeps the great trees, the Thornhollow dressing and the oakTree prop on the originals', () => {
    for (const file of [
      'jungle_features.ts',
      'realm_flora.ts',
      'garden_features.ts',
      'haunt_features.ts',
      'great_tree_prewarm.ts',
    ]) {
      expect(readFileSync(path.join(ROOT, 'src', 'render', file), 'utf8'), file).toContain(
