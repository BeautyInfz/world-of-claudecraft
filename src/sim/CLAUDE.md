...
| `account_ledger.ts` | the account ledger behind both books: deed id / relic key to the characters that earned or found it (`recordAccountDeed` / `recordAccountRelic`, dedupe per character, first day kept; `stampAccountDeedDay` for the join seed's own stamp), the union reads (`accountDeedLookup` / `accountRelicLookup` / `accountEarnedDays`), the catalog-bounded `acct` wire codec, and `accountLedgerRev`. INPUT to the sim: the server loads it per join and hands each session its own object (the fan-out copies entries between sessions); the offline host starts from `freshAccountLedger()`. Never serialized into `CharacterState`. |
| `deeds_restore.ts` | the Book of Deeds' two join-time passes, moved whole out of `Sim.addPlayer`: `restoreBookOfDeeds` (blob restore, renown recomputed, title and border re-validated against the account union) and `runBookOfDeedsJoinRetro` (discovery seed, retro fallbacks over the account union, full evaluation, then `seedAccountLedgerSelf`: own deeds with their blob day, own relics undated) |
