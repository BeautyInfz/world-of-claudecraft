...
async function encode(out, mode, resize, quality) {
  ...
  const transforms = [textureCompress({ encoder: sharp, targetFormat: 'png', formats: /^image\/webp$/, ...(resize ? { resize: [resize, resize] } : {}) })];
  transforms.push(toktx({ mode, slots: /baseColorTexture/, jobs: 2, encoder: sharp, ...(quality ? { quality, compression: 5 } : {}) }));
  transforms.push(meshopt({ encoder: MeshoptEncoder, level: 'high' }));
  await doc.transform(...transforms);
  const buf = Buffer.from(await io.writeBinary(doc)); fs.writeFileSync(out, buf);
  const j = JSON.parse(buf.toString('utf8', 20, 20 + buf.readUInt32LE(12)));
  const bv = j.bufferViews[j.images[0].bufferView];
  console.log(out.split('/').pop(), 'total', buf.length, 'texture', j.images[0].mimeType, bv.byteLength, 'ext', j.extensionsUsed.join('+'));
}
